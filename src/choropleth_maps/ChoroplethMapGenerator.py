...
class ChoroplethMapGenerator:
    ...
    def __init__(self, file_path: str):
        """
        Initializes the ChoroplethMapGenerator with the path to the company data CSV file.
        """
        self.file_path = file_path
        self.dataframe = None
        print(f"ChoroplethMapGenerator initialized for file: {self.file_path}")
...
def plot_state_political_leaning(visualizer: ChoroplethMapGenerator, state_names_df: pd.DataFrame,
                                 output_dir: str):
    """
...
def plot_average_beta(visualizer: ChoroplethMapGenerator, state_names_df: pd.DataFrame, output_dir: str):
    """
...
def plot_correlation(visualizer: ChoroplethMapGenerator, beta_df: pd.DataFrame, politics_df: pd.DataFrame,
                     output_dir: str):
    """
...
def main():
    ...
    output_dir = os.path.normpath(os.path.join(plots_dir, 'choropleth_maps'))
    os.makedirs(output_dir, exist_ok=True)
    company_data_file = os.path.join(data_dir, 'company_info.csv')

    visualizer = ChoroplethMapGenerator(company_data_file)
